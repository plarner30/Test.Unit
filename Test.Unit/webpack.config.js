...
module.exports = ({ production } = {}, {extractCss, analyze, tests, hmr, port, host } = {}) => {
  ...
  if (tests) cssRules[0].options = { url: false };
  ...
  return {
    ...
    output: {
      path: outDir,
      publicPath: production ? '/dist/' : baseUrl,
      ...(!tests ? {
    ...
    devServer: {
      static: outDir.replace('\dist', ''),
      // serve index.html for all 404 (required for push-state)
      historyApiFallback: true,
      hot: (!production && !tests) || hmr || project.platform.hmr,
    ...
    plugins: [
      ...
      // }),
      ...when(tests, new ProvidePlugin({
        process: 'process', // Installed via aurelia-cli
      })),
